  ...
  <div class="flex gap-2 items-center">
    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="myTab" v-if="!removeActive">
      <li class="mr-2" role="presentation" v-for="(b, id) in getTabs" :key="id" @click="changeActive(b.id)"
        ...
        <button class="inline-block p-3 rounded-t-lg" type="button" :class="(id == $store.state.activeTab && b.status && b.status) == 'Approved' ? 'border-b-2 text-green-600 bg-gradient-to-b from-[#ffffff]/[.35] from-[0%] to-[#15803D]/[.35] font-bold border-green-600' : (id == $store.state.activeTab && b.status && b.status) == 'Open' ? 'border-b-2 text-orange-600 bg-gradient-to-b from-[#ffffff]/[.35] from-[0%] to-[#FB923C]/[.35] font-bold border-orange-600' : (id == $store.state.activeTab && b.status && b.status) == 'Rejected' ? 'border-b-2 text-red-600 bg-gradient-to-b from-[#ffffff]/[.35] from-[0%] to-[#B91C1C]/[.35] font-bold border-red-600' 
