  ...
  <div class="flex gap-2 items-center">
    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="myTab">
      <li class="mr-2" role="presentation" v-for="(b, id) in getTabs" :key="id" @click="changeActive(b.id)"
        ...
        <button class="inline-block p-3 rounded-t-lg" type="button" :class="id == $store.state.activeTab
            ? 'border-b-2 violet-color hover:violet-color dark:text-[#753ED7] dark:hover:text-[#753ED7] border-[#753ED7] dark:border-[#753ED7]' : b.status && b.status == 'Approved' ? 'text-white bg-green-700' : b.status && b.status == 'Rejected' ? 'text-white bg-red-700' : b.status && b.status == 'Open' ? 'text-white bg-orange-400' 
