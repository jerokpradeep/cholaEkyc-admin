<template>
    ...
    <table class="bg-white w-full rounded-b border-t border-[#ededed] dark:border-[#232325] relative mt-[1px] rounded-lg">
            ...
            <tbody>
                <tr class="border-b cursor-pointer hover:bg-gray-50" v-for="(item, i) of list" :key="i">
                    ...
                    <td class="py-4 text-sm primary-color dark:text-[#94A3B8] relative flex justify-center">
                        <button @click="retryBo()" class="inline-block p-2 border  rounded " type="button"> <span class="flex gap-1 items-center" >
...
export default {
    ...
    methods: {
        retryBo(type) {
            // this.$store.dispatch('approval/retryBo')
        }
    },
}
</script>